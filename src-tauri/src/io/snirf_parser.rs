...
pub fn parse_snirf(path: &str) -> Result<Snirf, NWError> {
    parse_snirf_inner(path).map_err(NWError::from)
}

fn parse_snirf_inner(path: &str) -> Result<Snirf, NWError> {
    ...
    if nirs_entries.is_empty() {
        ...
        return Err(NWError::NoEntries.into());
    }
