...
<script>
  ...
  const EVENT_COLORS = [
    "#33cc66", "#ffaa22", "#22ccdd", "#dd44aa", "#aadd22", "#7766ff",
    "#ff6644", "#44ddaa", "#cc88ff", "#dddd33", "#ff88aa", "#6699ff",
  ];
  ...
  function buildMarkLines(xAxisIndex) {
    ...
    const visibleNames = new Set(eventTypes.filter((et) => et.visible).map((et) => et.name));
  ...
  function submitCursorTime() {
    ...
    const clamped = Math.max(timeArr[0], Math.min(timeArr[timeArr.length - 1], val));
  ...
  function onCursorKeydown(e) {
    if (e.key === "Enter") submitCursorTime();
  }
  ...
  function lttb(data, target) {
    ...
    for (let i = 1; i < target - 1; i++) {
      ...
      let avgX = 0, avgY = 0;
      for (let j = avgStart; j < avgEnd; j++) { avgX += data[j][0]; avgY += data[j][1]; }
      avgX /= avgEnd - avgStart;
      avgY /= avgEnd - avgStart;
      const rangeStart = Math.floor((i - 1) * bucketSize) + 1;
      const rangeEnd = Math.min(Math.floor(i * bucketSize) + 1, len);
      const px = data[prevIndex][0], py = data[prevIndex][1];
      let maxArea = -1, maxIdx = rangeStart;
      for (let j = rangeStart; j < rangeEnd; j++) {
        const area = Math.abs((px - avgX) * (data[j][1] - py) - (px - data[j][0]) * (avgY - py));
  ...
  function downsample(time, values) {
    if (time.length <= DOWNSAMPLE_TARGET) {
      ...
      for (let i = 0; i < time.length; i++) pairs[i] = [time[i], values[i]];
      return pairs;
    }
    const paired = new Array(time.length);
    for (let i = 0; i < time.length; i++) paired[i] = [time[i], values[i]];
  ...
  function updateChart() {
    ...
    if (channels.length === 0) {
      chart.setOption({
        backgroundColor: CHART_BG, animation: false,
        title: { text: "No channels selected", left: "center", top: "center", textStyle: { color: "#555", fontSize: 14 } },
        xAxis: [], yAxis: [], series: [], grid: [], dataZoom: [],
    ...
    const time = allData.time;
    if (stacked) buildStacked(channels, time);
    else buildUnstacked(channels, time);
  }

  const PERF_SERIES = {
    type: "line", symbol: "none", large: true,
    largeThreshold: 3000, progressive: 500, animation: false,
  };
  ...
  function buildStacked(channels, time) {
    ...
    channels.forEach((ch, idx) => {
      ...
      const hboSeries = {
        ...PERF_SERIES, name: hboName, data: downsample(time, ch.series_a),
        lineStyle: { color: HBO_COLOR, width: 1.5 }, itemStyle: { color: HBO_COLOR },
      };
      if (idx === 0) {
        hboSeries.markLine = { symbol: "none", silent: true, data: markLines };
        if (markAreas.length > 0) hboSeries.markArea = { silent: true, data: markAreas };
      }
      series.push(hboSeries);
      series.push({
        ...PERF_SERIES, name: hbrName, data: downsample(time, ch.series_b),
        lineStyle: { color: HBR_COLOR, width: 1.5, type: "dotted" }, itemStyle: { color: HBR_COLOR },
      });
    });
    chart.setOption({
  ...
  function buildUnstacked(channels, time) {
    ...
    channels.forEach((ch, i) => {
      ...
      xAxisIndices.push(i);
      yAxes.push({ type: "value", gridIndex: i, name: ch.name, nameLocation: "middle", nameGap: 55, nameTextStyle: { color: "#9090a0", fontSize: 11 }, axisLabel: { color: "#9090a0", formatter: (val) => val.toExponential(1) }, axisLine: { lineStyle: { color: CHART_AXIS } }, splitLine: { lineStyle: { color: CHART_GRID } } });
      const markLines = buildMarkLines(i);
      const markAreas = buildMarkAreas(i);
      const hboSeries = { ...PERF_SERIES, name: `${ch.name} HbO`, data: downsample(time, ch.series_a), xAxisIndex: i, yAxisIndex: i, lineStyle: { color: HBO_COLOR, width: 1.5 }, itemStyle: { color: HBO_COLOR }, markLine: { symbol: "none", silent: true, data: markLines } };
      if (markAreas.length > 0) hboSeries.markArea = { silent: true, data: markAreas };
      series.push(hboSeries);
      series.push({ ...PERF_SERIES, name: `${ch.name} HbR`, data: downsample(time, ch.series_b), xAxisIndex: i, yAxisIndex: i, lineStyle: { color: HBR_COLOR, width: 1.5, type: "dotted" }, itemStyle: { color: HBR_COLOR } });
    });
    chart.setOption({
      backgroundColor: "#0d0d18", animation: false, title: { show: false },
      tooltip: manyChannels ? { show: false } : { trigger: "axis", axisPointer: { type: "cross" } },
...
<div class="plotter-root">
  <div class="toolbar">
    ...
    {#if eventTypes.length > 0}
      ...
      <button class="toggle-btn" class:active={showEventsPanel} on:click={() => (showEventsPanel = !showEventsPanel)}>
        Events ({eventTypes.filter((e) => e.visible).length}/{eventTypes.length})
      </button>
    {/if}
    ...
    {#if allData}
      ...
      <input class="cursor-input" type="text" bind:value={cursorInputValue} on:keydown={onCursorKeydown} on:blur={submitCursorTime} placeholder="0.000" />
    ...
    <div class="events-panel">
      {#each eventTypes as et}
        <label class="event-toggle">
          <input type="checkbox" checked={et.visible} on:change={() => toggleEventType(et.name)} />
          <span class="event-swatch" style="background:{et.color}"></span>
          <span class="event-label">{et.name}</span>
          <span class="event-count">{allData?.events?.find((e) => e.name === et.name)?.markers.length ?? 0}</span>
        </label>
      {/each}
    </div>
  {/if}
...
<style>
  ...
  .chart-wrapper { flex: 1; min-height: 0; overflow: hidden; }
  .chart-container { width: 100%; height: 100%; }

  .toolbar-sep { width: 1px; height: 18px; background: var(--border-default); margin: 0 6px; }
  ...
  .event-toggle { display: flex; align-items: center; gap: 4px; cursor: pointer; font-size: 11px; color: var(--text-secondary); }
  .event-toggle input[type="checkbox"] { width: 12px; height: 12px; margin: 0; cursor: pointer; accent-color: var(--accent-green); }
  .event-swatch { display: inline-block; width: 10px; height: 10px; border-radius: 2px; }
  .event-label { color: var(--text-secondary); }
  .event-count { color: var(--text-muted); font-size: 10px; }

  .cursor-label { font-size: 11px; color: var(--text-muted); }
  .cursor-input {
    width: 72px;
    padding: 2px 6px;
    font-size: 12px;
  ...
  .cursor-input:focus { border-color: var(--accent-green); }
  .cursor-range { font-size: 11px; color: var(--text-muted); }
</style>
